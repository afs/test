...
public class RX {
    ...
    public static boolean MODE_SA = false;
    ...
    public static QueryIterator rdfStarTriple_SA(QueryIterator chain, Triple pattern, ExecutionContext execCxt) {
        ...
        return rdfStarTripleSub_SA(chain, pattern, execCxt);
    }

    private static QueryIterator rdfStarTripleSub_SA(QueryIterator input, Triple pattern, ExecutionContext execCxt) {
        return new QueryIterRepeatApply(input, execCxt) {
            @Override
            protected QueryIterator nextStage(Binding binding) {
                Iterator<Binding> matches = rdfStarTripleSub_SA(binding, pattern, execCxt);
                // Closes iterators.
    ...
    private static Iterator<Binding> rdfStarTripleSub_SA(Binding input, Triple tPattern, ExecutionContext execCxt) {
        ...
        ExtendedIterator<Binding> matched = graphIter.mapWith(tData->RX2.match(input, tData, tPattern)).filterDrop(Objects::isNull);
        return matched;
    }

    private static Node nodeTopLevel(Node node) {
        if ( Var.isVar(node) )
            return Node.ANY;
